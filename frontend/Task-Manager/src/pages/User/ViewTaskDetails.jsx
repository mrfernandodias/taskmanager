...
const ViewTaskDetails = () => {
  ...
  const getStatusTagColor = (status) => {
    switch (status) {
      case 'In Progress':
        return 'text-cyan-600 bg-cyan-50 border border-cyan-500/10';
      case 'Completed':
        return 'text-lime-600 bg-lime-50 border border-lime-500/10';
      default:
        return 'text-violet-600 bg-violet-50 border border-violet-500/10';
    }
  };
  ...
  // Get task info by id when component mounts or id changes
  useEffect(() => {
...
const InfoBox = ({ label, value }) => {
  return (
    <>
      ...
      <p className="text-[12px] md:text-[12px] font-medium text-gray-700 mt-0.5">{value}</p>
    </>
  );
};
